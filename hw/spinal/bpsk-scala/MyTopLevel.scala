...
// Hardware definition
class Convolution[I <: AFix, O <: AFix](inputDataType:  HardType[I], outputDataType:  HardType[O], width: Int) extends Component {
  val io = new Bundle {
  ...
  var coeff = Vec(AFix.S(2 exp, -12 exp), 43)
    coeff(0).raw := 186
    coeff(1).raw := 233
    coeff(2).raw := 248
    coeff(3).raw := 217
...
case class FirFilter() extends Component {
  ...
  var filter = new Convolution(AFix.S(2 exp, -12 exp), AFix.S(2 exp, -12 exp),  43)
